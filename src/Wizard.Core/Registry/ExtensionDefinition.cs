...
/// <summary>
/// One selectable Verify extension (plan 9.1). Hand-written data, sourced from the per-extension
/// research in plan-research/extension-catalogue-*.md.
/// </summary>
public sealed record ExtensionDefinition
{
    ...
    /// <summary>One line, shown on the extension card and as the first line of the guide's section.</summary>
    ...
    /// the extension has none, which means it can only be enabled explicitly (plan A1, A2).
    /// </summary>
    ...
    /// sensitively, and silently skips a miss (plan A1). An extension this returns false for is never
    ...
    /// <summary>Statements for the module initializer, in the order they must run within this extension.</summary>
    public IReadOnlyList<InitializeStatement> Initialize { get; init; } = [];

    /// <summary>Which block of the module initializer this extension's statements belong to.</summary>
    ...
    /// <summary>Namespaces the generated test file needs. Per file, never global: two extensions can
    ...
    /// <summary>Ids of <see cref="InteractionRules.Groups"/> this extension belongs to (plan 11.1).</summary>
    public IReadOnlyList<string> ExclusiveGroups { get; init; } = [];

    public IReadOnlyList<ExtensionChoice> Choices { get; init; } = [];
    ...
    public IReadOnlyList<ExtensionFile> LibraryFiles { get; init; } = [];
...
public sealed record InitializeStatement(string Code, params string[] Comment)
{
    ...
    /// registration the extension makes in one role but not another.
    /// </summary>
...
public sealed record ExtensionChoice(
    string Id,
    string Label,
    string Hint,
...
public sealed record ExtensionFile(string Path, string Content);
