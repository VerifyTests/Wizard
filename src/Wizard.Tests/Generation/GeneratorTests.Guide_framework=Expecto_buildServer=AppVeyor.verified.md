...
Generated by the [Verify wizard](https://verifytests.github.io/Wizard/new?step=output&os=Windows&ide=Rider&cli=Cli&tf=Expecto&ci=AppVeyor&sponsor=Exempt&exempt=OpenSource&until=2027-09) (reopen that link to change any answer).
...
Verify is configured once, before the first verification. F# has no module initializers, so `Tests.fs` holds a lazy value that each test forces first:
...
`ClassBeingTested.FindPerson()` lives in the `VerifySample` project. The solution includes the approved snapshot, `Tests.findPerson.verified.txt`, so this test passes on the first run:
...
Expecto tests pass their name to `Verifier.Verify`, which becomes the snapshot's file name.
