...
Generated by the [Verify wizard](https://verifytests.github.io/Wizard/new?step=output&os=Windows&ide=Rider&cli=Cli&tf=Expecto&ci=GitHubActions&sponsor=Exempt&exempt=OpenSource&until=2027-09). This project was set up with Verify snapshot testing; the facts below describe that setup.

## Project facts

 * Solution: `VerifySample.slnx`. Code under test is in `src/VerifySample`, tests in `src/VerifySample.Tests`.
 * Test framework: Expecto (tests in F#, code under test in C#).
 ...
 * Verify packages: Verify.Expecto 33.1.1, Verify.DiffPlex 3.3.1.
 * Verify is configured by the lazy `initialize` value in `Tests.fs`; every test forces it before verifying.
