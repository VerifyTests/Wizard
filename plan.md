...
| A8 | Verify.Pandoc depends on the `Pandoc` package, which ships a second SponsorCheck gate, for owner `Papyrine`. Selecting it without those properties fails the build. | Recorded as an external requirement and in the extension's notes. | — |
...
| A16 | The cases where a wrong ordering only shows up at compile or run time. | Added to the integration matrix: Blazor with the core sample, QuestPDF with PdfPig, Pandoc with the Papyrine properties, Flurl with Http, ImageMagick comparer-only with PdfPig. | — |
