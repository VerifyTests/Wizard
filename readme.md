 ...
 * https://verifytests.github.io/Wizard/add adds extensions to an existing project.
 * `https://verifytests.github.io/Wizard/add/{ExtensionId}` adds one extension, already selected. An
   extension's readme can link here, for example https://verifytests.github.io/Wizard/add/EntityFramework.

The browser remembers the tech stack, the extensions already in use and the sponsor answers between
visits. The home page has a button that forgets them.


## Adding an extension

 1. Add an `ExtensionDefinition` to the `src/Wizard.Core/Registry/Extensions.<A-D>.cs` file for its
    catalogue. The xml docs in `ExtensionDefinition.cs` describe each member, and `RegistryTests` checks
    most of them.
 ...
 3. If it conflicts with or depends on another extension, add a rule to
 ...
 * `integration.yml` builds and tests a generated solution per extension and per test framework, nightly
